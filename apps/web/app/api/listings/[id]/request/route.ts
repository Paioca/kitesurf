...
const schema = z.object({ type: z.enum(['offer', 'visit']), amount: z.number().int().min(100).optional() });
...
export async function POST(req: Request, { params }: { params: { id: string } }) {
  try {
    ...
    const r = await createRequest(user.id, params.id, parsed.data.type, parsed.data.amount);
